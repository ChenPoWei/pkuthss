	 		...
	 		\begin{enumerate}
				...
				\begin{enumerate}
					...
					\begin{enumerate}
						...
						\item 周边设备：一台以上手持移动装置，以手机为例，与用来代表虚拟商品的数个RFID标签；已可供测试NFC用的手持移动装置包含小米3 WCDMA版，详细规格于表\ref{mi}，Google Nexus 5X，详细规格于表\ref{5x}。
...
%			\end{enumerate}
		...
		\subsection{测试用例}
			...
			\item 集成测试（Integration Test）
				...
				\item IT3测试用例：
					表\ref{IT3TestCase}为IT3 测试用例，目标测试对象为客户端行动支付和交易子系统。目的：验证［CMPTSS1.3.0］能正确接收SMCTSS所发送的交易数据，并以其交易信息运行以比特币付款之动作。可以查找商品信息，且能够存储并且查看用户过往之交易纪录。
